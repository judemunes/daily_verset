...
def send_daily_notification() -> None:
    """Envoie la notification push quotidienne avec le verset du jour."""
...
def on_startup() -> None:
    """Initialise la base de données et démarre le scheduler de notifications."""
    init_db(DB_FILE)
    scheduler.add_job(
...
def api_vapid_public_key() -> dict[str, str]:
    """Renvoie la clé publique VAPID pour les notifications push."""
...
def api_list_comments() -> list[dict]:
    """Renvoie tous les commentaires avec leurs réactions et réponses."""
...
async def api_edit_comment(comment_id: int, request: Request) -> JSONResponse:
    """Modifie un commentaire existant (dans les 5 minutes)."""
    body = await request.json()
    try:
        result = edit_comment(
            comment_id,
...
async def api_delete_comment(comment_id: int, request: Request) -> JSONResponse:
    """Supprime un commentaire et ses réponses/réactions."""
    body = await request.json()
    edit_token = body.get("edit_token", "")
...
async def api_react_comment(comment_id: int, request: Request) -> JSONResponse:
    """Ajoute ou incrémente une réaction sur un commentaire."""
    body = await request.json()
    try:
...
async def api_unreact_comment(comment_id: int, request: Request) -> JSONResponse:
    """Retire ou décrémente une réaction sur un commentaire."""
    body = await request.json()
    try:
...
def serve_sw() -> FileResponse:
    """Sert le service worker à la racine (nécessaire pour le scope /)."""
    return FileResponse(os.path.join(BASE_DIR, "sw.js"), media_type="application/javascript")


@app.get("/favicon.ico")
def favicon() -> FileResponse:
    """Évite l'erreur 404 sur le favicon."""
    return FileResponse(os.path.join(BASE_DIR, "static", "favicon.ico"), status_code=204)
